 ...
 * 
 * Bond1Run0_1: Test case with 2 atoms & 1 bond & coeff_argv{1, 132.0, 3.0, 2.0}
 * Bond1Run0_2: Test case with 2 atoms & 1 bond & coeff_argv{1, 500.0, 5.0, 10.0}
 * 
 * Bond2Run0_1: Test case with 4 atoms & 2 bonds & coeff_argv{1, 100.0, 5.0, 1.0}
 * Bond2Run0_2: Test case with 4 atoms & 2 bond & coeff_argv{1, 50.0, 2.0, 5.0}
 * 
 * Bond3Run0_1: Test case with 4 atoms & 3 bond & coeff_argv{1, 100.0, 3.0, 1}
 * Bond3Run0_2: Test case with 4 atoms & 3 bond & coeff_argv{1, 50.0, 9.0, 5.0}
 *
 */

#include "test_bond_morse.h"
...
namespace {
  ...
  class BondMorseTest : public ::testing::Test {   
  protected:
    BondMorseTest() {
      char *argv[] = {"bond_morse", "-screen", "none", "-log", "none", NULL};
 ...
 TEST_F(BondMorseTest, Coeff_CorrectNarg){
    lmp->force->bond = new BondMorse(lmp);
    ...
    for(i = 1; i <=  lmp->atom->nbondtypes; i++){
      EXPECT_DOUBLE_EQ(5.0, lmp->force->bond->equilibrium_distance(i));
 ...
 TEST_F(BondMorseTest, Coeff_highNarg){
    lmp->force->bond = new BondMorse(lmp);
    char *bcoeff_argv[] = {"1", "100.0", "2.5", "5.0", NULL};
    int bcoeff_narg; 
 ...
 TEST_F(BondMorseTest, Coeff_lowNarg){
    lmp->force->bond = new BondMorse(lmp);
    char *bcoeff_argv[] = {"1", "100.0", "2.5", "5.0", NULL};
    int bcoeff_narg; 
  ...
  TEST_F(BondMorseTest,Coeff_NegativeNarg){
    lmp->force->bond = new BondMorse(lmp);
    char *bcoeff_argv[] = {"1", "100.0", "2.5", "5.0", NULL};
    int bcoeff_narg; 
 ...
 //////////////////////////////////////////////////////////////////////
  ...
  // Test basic case with 2 atoms & 1 bond & coeff_argv{1, d0 = 132.0, alpha = 3.0, r_0 = 2.0}
  TEST_F(BondMorseTest, Bond1Run0_1) {
    // Variables for test
    double d0 = 132.0;
    double alpha = 3.0;
    double r0 = 2.0;
    ...
    double r = sqrt(rsq);
    double dr = r - r0;
    double ralpha = exp(-alpha*dr);
    double fbond = -2.0*d0*alpha*(1-ralpha)*ralpha/r; 
    double ebond = d0*(1-ralpha)*(1-ralpha); 
  ...
  TEST_F(BondMorseTest, Bond1Run0_2) {
     // Variables for test
    double d0 = 500.0;
    double alpha = 5.0;
    double r0 = 10.0;
  ...
  TEST_F(BondMorseTest, Bond2Run0_1) {     
    // Variables for run 
    double d0 = 100.0;
    double alpha = 5.0;
    double r0 = 1.0;
    ...
    double r1 = sqrt(rsq1);
    double dr1 = r1 - r0;
    double ralpha1 = exp(-alpha*dr1);
    double fbond1 = -2.0*d0*alpha*(1-ralpha1)*ralpha1/r1; 
    double ebond1 = d0*(1-ralpha1)*(1-ralpha1);   
    ...
    double r2 = sqrt(rsq2);
    double dr2 = r2 - r0;
    double ralpha2 = exp(-alpha*dr2);
    double fbond2 = -2.0*d0*alpha*(1-ralpha2)*ralpha2/r2; 
    double ebond2 = d0*(1-ralpha2)*(1-ralpha2);   
  ...
  TEST_F(BondMorseTest, Bond2Run0_2) {     
    // Variables for run 
    double d0 = 50.0;
    double alpha = 2.0;
    double r0 = 5.0;
  ...
  TEST_F(BondMorseTest, Bond3Run0_1) {     
    // Variables for run 
    double d0 = 100.0;
    double alpha = 3.0;
    double r0 = 1.0;
    ...
    double r3 = sqrt(rsq3);
    double dr3 = r3 - r0;
    double ralpha3 = exp(-alpha*dr3);
    double fbond3 = -2.0*d0*alpha*(1-ralpha3)*ralpha3/r3; 
    double ebond3 = d0*(1-ralpha3)*(1-ralpha3);   
    ...
    lmp->force->bond = new BondMorse(lmp);
    //  k = 100.0 and r0 = 1
    char *bcoeff_argv[] = {"1", "100.0", "3.0", "1", NULL};
    int bcoeff_narg; 
  ...
  TEST_F(BondMorseTest, Bond3Run0_2) {     
    // Variables for run 
    double d0 = 50.0;
    double alpha = 9.0;
    double r0 = 5.0;
